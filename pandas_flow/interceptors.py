...
from .events import OperationType, DataFrameInfo
...
def _intercept_concat() -> None:
    ...
    def tracked_concat(objs, *args, **kwargs) -> pd.DataFrame:
        global _tracking_active
        
        result = _original_methods["pd.concat"](objs, *args, **kwargs)
        
        # Skip if tracker not active or if we're inside another tracked operation
        if _tracker and not _tracking_active:
    ...
    def tracked_pivot_table(self, *args, **kwargs) -> pd.DataFrame:
        ...
        if _tracker:
            ...
            _tracker.record_operation(
                ...
                arguments={"index": index, "columns": columns, "values": values, "aggfunc": str(aggfunc)},
            )
