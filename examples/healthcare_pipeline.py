...
import pandas_flow
from pandas_flow.interceptors import track_filter, track_groupby_agg
...
def generate_healthcare_data():
    ...
    patients = pd.DataFrame({
        ...
        "insurance_type": np.random.choice(["Private", "Medicare", "Medicaid", "None"], n_patients, p=[0.5, 0.25, 0.15, 0.1]),
    })
    ...
    visits = pd.DataFrame({
        ...
        "visit_type": np.random.choice(["Inpatient", "Outpatient", "Emergency"], n_visits, p=[0.2, 0.5, 0.3]),
    ...
    labs = pd.DataFrame({
        ...
        "test_name": np.random.choice(
            ["CBC", "BMP", "Lipid Panel", "HbA1c", "TSH", "Urinalysis", "CRP"],
            n_labs
        ),
    ...
    diagnoses = pd.DataFrame({
        ...
        "description": np.random.choice([
            ...
            "Back Pain", "GERD", "Depression", "UTI", "Asthma", "Migraine", "Dermatitis"
        ], n_diagnoses),
